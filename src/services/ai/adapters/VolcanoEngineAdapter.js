...
import { generateCardsWithVolcano } from '../../volcanoEngine'
import { httpPost } from '../../../utils/httpClient'
import { VOLCANO_ENGINE_API_URL, KNOWLEDGE_POINT_EXTRACTION_PROMPT } from '../../../utils/constants'
import { logAiCall, resolveModelName } from '../../aiCallLog'

export class VolcanoEngineAdapter extends AiServiceInterface {
  constructor(config) {
    super()
    this.config = config
    this.apiKey = config.volcanoApiKey
    this.model = config.model || 'doubao-pro-32k'
  }

  getModelType() { return MODEL_TYPES.STRONG }
  supportsBatchProcessing() { return false }

  async extractKnowledgePoints(text, options = {}) {
    const startTime = Date.now()
    try {
      const prompt = KNOWLEDGE_POINT_EXTRACTION_PROMPT.replace('{{text}}', text)
      const body = { model: this.model, messages: [{ role: 'user', content: prompt }], temperature: 0.3, max_tokens: 4096 }
      const resp = await httpPost(VOLCANO_ENGINE_API_URL, { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + this.apiKey }, data: body, timeout: options.timeout || 30000 })
      if (!resp.ok) throw new Error('Volcano: 请求失败')
      const raw = resp.data?.choices?.[0]?.message?.content || ''
      const result = this._parseKnowledgePoints(raw)
      logAiCall({ purpose: 'knowledge-point-extraction', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'success', tokens: resp.data?.usage?.total_tokens || 0, prompt: text, response: raw })
      return result
    } catch (err) {
      logAiCall({ purpose: 'knowledge-point-extraction', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'error', errorMessage: err?.message || '未知错误', prompt: text, response: '' })
      throw err
    }
  }
  ...
  async clusterKnowledgePoints(knowledgePoints, options = {}) {
    ...
    try {
      ...
      const body = { model: this.model, messages: [{ role: 'user', content: topicPrompt }], temperature: 0.3, max_tokens: 4096 }
      const resp = await httpPost(VOLCANO_ENGINE_API_URL, { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + this.apiKey }, data: body, timeout: options.timeout || 30000 })
      if (!resp.ok) throw new Error('Volcano: 请求失败')
      const raw = resp.data?.choices?.[0]?.message?.content || ''
      const result = this._parseClusterResult(raw, knowledgePoints)
      logAiCall({ purpose: 'topic-clustering', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'success', tokens: resp.data?.usage?.total_tokens || 0, prompt: topicPrompt, response: raw })
      return result
    } catch (err) {
      logAiCall({ purpose: 'topic-clustering', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'error', errorMessage: err?.message || '未知错误', prompt: '', response: '' })
      throw err
    }
  }
  ...
  async reorganizeUnits(categoryName, allCards, existingUnitNames, options = {}) {
    ...
    try {
      ...
      logAiCall({ purpose: 'unit-reorganization', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'success', tokens: resp.data?.usage?.total_tokens || 0, prompt: prompt, response: raw })
      return result
    } catch (err) {
      logAiCall({ purpose: 'unit-reorganization', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'error', errorMessage: err?.message || '未知错误', prompt: '', response: '' })
      throw err
    }
  }
  ...
  async generateCards(knowledgePoints, options = {}) {
    ...
    try {
      ...
      const result = await generateCardsWithVolcano(knowledgePoints.join('\n'), this.apiKey, this.model, summaryLevel)
      logAiCall({ purpose: 'card-generation', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'success', tokens: 0, prompt: knowledgePoints.join('\n'), response: result })
      return result
    } catch (err) {
      logAiCall({ purpose: 'card-generation', modelName: resolveModelName('volcano', this.model), durationMs: Date.now() - startTime, status: 'error', errorMessage: err?.message || '未知错误', prompt: '', response: '' })
      throw err
    }
  }
  ...
  _parseKnowledgePoints(raw) {
    try {
      ...
      if (firstBracket !== -1 && lastBracket > firstBracket) {
        ...
        if (Array.isArray(parsed)) return parsed.map(p => typeof p === 'string' ? p.trim() : String(p).trim()).filter(p => p.length > 0)
      }
    } catch (_) {}
    return []
  }
  ...
  _parseClusterResult(raw, knowledgePoints) {
    try {
      ...
      if (firstBracket !== -1 && lastBracket > firstBracket) {
        const jsonStr = trimmed.slice(firstBracket, lastBracket + 1)
        const parsed = JSON.parse(jsonStr)
        if (Array.isArray(parsed)) return parsed.map(topic => ({ name: String(topic.name || topic).trim(), cardIndices: Array.isArray(topic.cardIndices) ? topic.cardIndices : [] }))
      }
    } catch (_) {}
    return []
  }
  ...
  _buildTopicClusterPrompt(knowledgePoints, granularity) {
    const granularityDesc = { concise: '更粗粒度，生成 3-5 个主题', standard: '标准粒度，生成 4-8 个主题', detailed: '更细粒度，生成 6-12 个主题' }[granularity] || '标准粒度，生成 4-8 个主题'
...
注意：cardIndices 是知识点在输入列表中的下标（从 0 开始）。`
  ...
  _buildReorganizePrompt(categoryName, allCards, existingUnitNames, existingChapters) {
    ...
    const existingHint = existingUnitNames && existingUnitNames.length > 0 ? `【现有单元名称（供参考）】：${existingUnitNames.map(n => '"' + n + '"').join('、')}` : '（当前分类下尚无单元）'
    const hasChapters = existingChapters && existingChapters.length > 0
    const chapterHint = hasChapters ? `【现有章节（供参考）】：${existingChapters.map(ch => '"' + ch.name + '"').join('、')}` : ''
