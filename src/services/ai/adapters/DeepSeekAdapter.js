...
import { generateCards as dsGenerateCards } from '../../deepseek'
import { httpPost } from '../../../utils/httpClient'
import { DEEPSEEK_API_URL, KNOWLEDGE_POINT_EXTRACTION_PROMPT } from '../../../utils/constants'
import { logAiCall, resolveModelName } from '../../aiCallLog'

export class DeepSeekAdapter extends AiServiceInterface {
  constructor(config) {
    super()
    this.config = config
  ...
  async extractKnowledgePoints(text, options = {}) {
    ...
    try {
      ...
      logAiCall({
        ...
        modelName: resolveModelName('deepseek', this.model),
  ...
  async clusterKnowledgePoints(knowledgePoints, options = {}) {
    ...
    try {
      ...
      const topicPrompt = this._buildTopicClusterPrompt(knowledgePoints, granularity, previousTopics)
      const body = {
        model: this.model,
  ...
  async generateCards(knowledgePoints, options = {}) {
    ...
    try {
      ...
      const result = await dsGenerateCards(knowledgePoints.join('\n'), this.apiKey, this.model, summaryLevel)
  ...
  _buildTopicClusterPrompt(knowledgePoints, granularity, previousTopics) {
    ...
    let prompt = `你是考研科目的知识体系整理助手。请阅读以下知识点列表，然后按照内容的逻辑关联进行主题聚类。

【任务】：
将知识点划分为若干主题，每个主题下包含若干相关的知识点。
...
注意：cardIndices 是知识点在输入列表中的下标（从 0 开始）。`
    ...
    if (previousTopics && previousTopics.length > 0) {
      const prevTopicsText = previousTopics.map(t => t.name).join('、')
      prompt += `

【参考信息】：
之前已有的主题名称：${prevTopicsText}
请尽量保持主题命名风格一致。`
    }
