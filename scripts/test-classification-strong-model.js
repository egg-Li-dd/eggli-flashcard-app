/**
 * 强模型（DeepSeek）分类逻辑测试脚本
 * 直接调用 DeepSeek API，验证强模型分类是否正确
 *
 * 使用方法:
 *   node scripts/test-classification-strong-model.js
 *
...
const TOPIC_GROUP_KEYWORDS = {
  '计算机系统组成概述': ['CPU', '运算器', '控制器', '存储器', '输入设备', '输出设备', '裸机', '软硬件', '系统组成', '硬件部件', '软件系统', '软件特点', '硬件系统', '软件解析', '系统构成', '运行原理', '部件', '功能分工'],
  '计算机发展与分类': ['ENIAC', '发展', '电子管', '晶体管', '集成电路', '巨型机', '大型机', '小型机', '微型机', '嵌入式', '特性', '用途', '发展方向', '巨型化', '微型化', '网络化', '智能化', '多媒体', '演进', '分类'],
  '计算机数制与信息编码': ['二进制', '八进制', '十进制', '十六进制', '进制', '字节', 'Byte', 'KB', 'MB', 'GB', 'TB', '位', 'bit', 'ASCII', 'GB2312', '汉字', '编码', '字母', '除2取余', '存储单位', '换算', '数制', '信息表示'],
  '内部存储硬件': ['RAM', 'ROM', 'Cache', '高速缓存', 'DDR', '外存', '机械硬盘', 'HDD', '固态硬盘', 'SSD', 'U盘', '光盘', '移动外存', '读写速度', '断电', '随机存取', '只读存储', '存储层级', '外设', '硬件体系'],
}
...
// ===== 主测试函数 =====
async function main() {
  console.log('🧪 强模型（DeepSeek）智能单元整理分类逻辑测试')
  console.log(`📅 时间: ${new Date().toISOString()}`)
  console.log(`🤖 AI 服务: DeepSeek (强模型)`)
  ...
  for (const mode of modes) {
    ...
    console.log(`  章节准确率: ${r.stats.cardCount > 0 ? (r.stats.correctChapters / r.stats.cardCount * 100).toFixed(1) : 0}%`)
